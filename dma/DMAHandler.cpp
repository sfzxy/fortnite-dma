...
bool DMAHandler::FixDTB() {
	...
	bool result = VMMDLL_Map_GetModuleFromNameU(this->DMA_HANDLE, this->GetPID(), (LPCSTR)this->GameName.c_str(), &moduleEntry, NULL);
	if (result)
	...
	SetConsoleTitleA("Asgard (fixing dtb.. step 1)");
	...
	while (true) {
		...
		if (nt == VMMDLL_STATUS_SUCCESS) {
			...
			SetConsoleTitleA(("Asgard (fixing dtb.. step 1 " + content + "% )").c_str());
		}

		std::this_thread::sleep_for(std::chrono::milliseconds(100));
	}

	SetConsoleTitleA("Asgard (fixing dtb.. step 2)");

	VMMDLL_VFS_FILELIST2 VfsFileList;
	VfsFileList.dwVersion = VMMDLL_VFS_FILELIST_VERSION;
	VfsFileList.h = 0;
	VfsFileList.pfnAddDirectory = 0;
	VfsFileList.pfnAddFile = cbAddFile; // Dumb callback

	result = VMMDLL_VfsListU(this->DMA_HANDLE, (LPCSTR)"\\misc\\procinfo\\", &VfsFileList);
	if (!result)
		return false;

	SetConsoleTitleA("Asgard (fixing dtb.. step 3)");
	...
	SetConsoleTitleA("Asgard (fixing dtb.. step 4)");

	std::vector<uint64_t> possibleDTBs;
	char* pLineStart = reinterpret_cast<char*>(bytes);
	for (size_t i = 0; i < 1000; ++i) { // Loop over it 1000 times, assuming they'll never have more than 1k processes ran at the same time
	...
	SetConsoleTitleA("Asgard (fixing dtb.. step 5)");

	// Loop over possible dtbs and set the config to use it until we find the correct one
	for (size_t i = 0; i < possibleDTBs.size(); i++) {
		...
		result = VMMDLL_Map_GetModuleFromNameU(this->DMA_HANDLE, this->GetPID(), (LPCSTR)this->GameName.c_str(), &moduleEntry, NULL);
		if (result) {
			delete[] bytes;
			SetConsoleTitleA("Asgard (dtb fixed)");
			return true;
		}
	}
...
void DMAHandler::assertNoInit() const {
	if (!DMA_HANDLE || !PROCESS_INITIALIZED) {
		log("DMA or process not initialized!");
		throw new std::string("DMA not initialized or process handle is invalid!");
	}
}
...
DMAHandler::DMAHandler(const wchar_t* wname, bool memMap)
{
	...
	// Convert the wide string to a standard string because VMMDLL_PidGetFromName expects LPCSTR.
	std::wstring ws(wname);
	const std::string str(ws.begin(), ws.end());

	processInfo.name = str;
	processInfo.wname = wname;
	if (!VMMDLL_PidGetFromName(DMA_HANDLE, processInfo.name.c_str(), &processInfo.pid))
	{
...
VMMDLL_SCATTER_HANDLE DMAHandler::CreateScatterHandle(int pid) const
{
	//assertNoInit();
