...
bool ImGui::Keybind(const char* label, int* key, int* mode)
{
    ...
    ImRect clickable(ImVec2(rect.Max.x - 29 - label_size.x, rect.Min.y), rect.Max);
    ...
    window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow + rect.Min.x, clickable.Min.y), clickable.Max - ImVec2(0, 0), GetColorU32(c::keybind::background_inactive), c::keybind::rounding);

    GetWindowDrawList()->AddImage(pic::widget, ImVec2(it_anim->second.slow + rect.Min.x + 2, clickable.Min.y + 2), clickable.Min + ImVec2(20, 20), ImVec2(0, 0), ImVec2(1, 1), ImGui::GetColorU32(c::keybind::icon));
    ...
    window->DrawList->AddText(rect.Min, GetColorU32(it_anim->second.text), label);
    ...
    RenderTextClipped(clickable.Min + ImVec2(17, 0), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
    ImGui::PopClipRect();
    ...
    if (it_anim->second.alpha >= 0.01f)
    {
        ...
        SetNextWindowSize(ImVec2(69 + 17, 85));
...
bool ImGui::Tabs(bool selected, ImTextureID image, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::tabs::background_active : c::tabs::background_inactive, g.IO.DeltaTime * 6.f);
...
bool ImGui::SubTab(bool selected, ImTextureID image, const ImVec2& size_arg)
{
    ...
    it_anim->second.image = ImLerp(it_anim->second.image, selected || hovered ? c::accent : c::text::text, g.IO.DeltaTime * 12.f);

    for (int i = 0; i < c::text::glow_size; i++) {
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 1.f / i / 12);
        ImGui::GetWindowDrawList()->AddImage(image, bb.Min - ImVec2(i, i), bb.Max - ImVec2(i, i), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.image));
        ImGui::GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(i, i), bb.Max + ImVec2(i, i), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.image));
        ImGui::PopStyleVar();
    }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    invisible_shadow(1.f, bb.Min, bb.Max, GetColorU32(c::accent), 300);

    ImGui::GetWindowDrawList()->AddRect(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), GetColorU32(c::child::background), c::button::rounding, NULL, 2.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(5, -5), grab_rect.Max - ImVec2(5, 5), GetColorU32(c::accent), 30.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::accent : c::checkbox::circle, g.IO.DeltaTime * 6.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 27) + 12 : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.line = ImLerp(it_anim->second.line, it_anim->second.opened_combo ? 2.f : 0.f, g.IO.DeltaTime * 12.f);
    ...
    if (it_anim->second.line > 0.1f)
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::accent), c::input::rounding + 2);

    GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y - it_anim->second.line), GetColorU32(c::combo::background), c::combo::rounding, ImDrawFlags_RoundCornersLeft);
    GetWindowDrawList()->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max - ImVec2(0, it_anim->second.line), GetColorU32(c::combo::background), c::combo::rounding, ImDrawFlags_RoundCornersRight);
    GetWindowDrawList()->AddImage(pic::widget, bb.Max - ImVec2(30, 40 - 6), bb.Max - ImVec2(6, 8), ImVec2(0, 0), ImVec2(1, 1), ImGui::GetColorU32(c::combo::icon));
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(5, 0), bb.Min + ImVec2(CalcTextSize(label).x + 16, 9), GetColorU32(c::child::background), 8.f, ImDrawCornerFlags_Bot);

    ImGui::GlowText(fonts::inter_bold, 17.f, label, ImVec2(bb.Max.x - (w - 9), bb.Min.y - 9), GetColorU32(c::accent));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 6), window->DC.CursorPos + ImVec2(w, label_size.y + 21.f));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddShadowRect(frame_bb.Min + ImVec2(0, 6), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, grab_bb.Max.y - 4), GetColorU32(c::accent), 10.f, ImVec2(0, 0), 100.f);
    ...
    ImGui::GlowText(fonts::inter_bold, 17.f, value_buf, ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y - 22), GetColorU32(c::accent));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        if (it_anim->second.line > 0.1f)
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::accent), c::input::rounding + 2);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(0, it_anim->second.line), GetColorU32(c::input::background), c::input::rounding);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(5, 0), frame_bb.Min + ImVec2(CalcTextSize(label).x + 16, 9), GetColorU32(c::child::background), 8.f, ImDrawCornerFlags_Bot);

        GetWindowDrawList()->AddImage(pic::widget, frame_bb.Max - ImVec2(30, size_arg.y - 6), frame_bb.Max - ImVec2(6, 8), ImVec2(0, 0), ImVec2(1, 1), ImGui::GetColorU32(c::input::icon));
    }
    ...
    ImGui::GlowText(fonts::inter_bold, 17.f, label, ImVec2(frame_bb.Max.x - (size_arg.x - 9), frame_bb.Min.y - 9), GetColorU32(c::accent));
